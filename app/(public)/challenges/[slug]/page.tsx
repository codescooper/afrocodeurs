...
import { submitDraftChallengeAction, unlockHintAction } from "@/features/challenges/actions";
import { CHALLENGE_DIFFICULTY_LABELS, CHALLENGE_STATUS_LABELS } from "@/features/challenges/constants";
import { PixelTerminalGame } from "@/features/challenges/pixel-terminal-game";
...
export default async function ChallengePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const engineProgress = session?.user && challenge.mode === "PIXEL_TERMINAL"
    ? await db.challengeProgress.findUnique({ where: { challengeId_userId: { challengeId: challenge.id, userId: session.user.id } } })
    : null;
  ...
  return <div className="mx-auto w-full max-w-4xl px-4 py-12"><div className="flex flex-wrap items-center justify-between gap-3"><Link href="/challenges" className="text-sm text-muted-foreground hover:underline">← Tous les défis</Link>{author && <Link href="/dashboard/contributions" className="text-sm font-medium underline">Mes énigmes</Link>}</div><div className="mt-5 flex flex-wrap items-center gap-2"><span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">{CHALLENGE_DIFFICULTY_LABELS[challenge.difficulty]}</span><span className="rounded-full bg-muted px-3 py-1 text-xs">{CHALLENGE_STATUS_LABELS[challenge.status]}</span><span className="font-bold text-primary">{challenge.basePoints} points</span></div><h1 className="mt-3 text-4xl font-bold tracking-tight">{challenge.title}</h1><p className="mt-2 text-sm text-muted-foreground">Imaginée par {challenge.author.name ?? `@${challenge.author.username}`} · {challenge.solves.length} résolution(s)</p>{author && challenge.status === "DRAFT" && <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-300 bg-amber-50 p-4 text-amber-950"><div><strong>Brouillon privé</strong><p className="text-sm">Vous seul pouvez voir cette énigme. Soumettez-la lorsque votre test est terminé.</p></div><form action={submitDraftChallengeAction}><input type="hidden" name="id" value={challenge.id} /><Button type="submit">Soumettre à la modération</Button></form></div>}{challenge.story && <div className="mt-8 rounded-xl border border-primary/20 bg-primary/5 p-5 text-lg italic">{challenge.story}</div>}<article className="mt-8 rounded-xl border border-border p-6"><Markdown>{challenge.instructions}</Markdown></article>
  {session?.user && open && !solve && challenge.mode === "PIXEL_TERMINAL" && <PixelTerminalGame challengeId={challenge.id} initial={{ step: engineProgress?.step ?? 0, inventory: engineProgress?.inventory ?? [], message: "Le soleil décline sur l’atelier. Explorez la scène.", terminal: [], completionCode: engineProgress?.step === 8 ? challenge.completionCode ?? undefined : undefined }} />}
  {session?.user && open && !solve && challenge.mode === "CLASSIC" && <div className="mt-6"><AnswerForm id={challenge.id} /><p className="mt-2 text-xs text-muted-foreground">{attempts}/{challenge.maxAttempts} tentatives utilisées · 5 tentatives maximum par minute.</p></div>}
