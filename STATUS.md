...
**Contenu fondateur** : peupler la prod (vide) avec de vrais premiers problèmes / ressources / communautés — ce qui résout le cold-start. En parallèle, **activer la prod** dans les variables Railway : clés Resend (+ domaine vérifié), Turnstile, `CRON_SECRET` (+ planifier le cron digest hebdo).
