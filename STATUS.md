...
**Activer la prod** : dans les variables Railway, ajouter les clés **Resend** (+ domaine vérifié), **Turnstile**, `CRON_SECRET` (+ planifier les crons `/api/cron/digest` hebdo **et** `/api/cron/roadmap-sync` quotidien ; poser `GITHUB_TOKEN` pour le quota GitHub). Puis réclamer le compte officiel `@afrocodeurs` (mot de passe via « mot de passe oublié » une fois l'email actif).
...
- **Prod en mode dégradé** : Resend (email) et Turnstile (CAPTCHA) **inactifs** tant que les clés ne sont pas dans les variables Railway → pas de récupération de compte par email ; inscription non protégée. Web Push aussi inactif (clés VAPID non définies en prod)
