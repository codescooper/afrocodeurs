...
**Activer la prod** : dans les variables Railway, ajouter les clés **Resend** (+ domaine vérifié), **Turnstile**, `CRON_SECRET` (+ planifier le cron `/api/cron/digest` hebdo). Puis réclamer le compte officiel `@afrocodeurs` (mot de passe via « mot de passe oublié » une fois l'email actif).
